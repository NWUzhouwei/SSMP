...
class Encoder(nn.Module):   ## Embedding module
    def __init__(self, encoder_channel,input):
        super().__init__()
        self.encoder_channel = encoder_channel
        self.first_conv = nn.Sequential(
            nn.Conv1d(input, 128, 1),
            nn.BatchNorm1d(128),
...
class PointTransformer(nn.Module):
    def __init__(self,input,out ,**kwargs):
        super().__init__()
        self.group_divider = Group(num_group=64, group_size=32)
        ...
        self.encoder = Encoder(encoder_channel=384,input=input)
        self.mlp=nn.Linear(384,out)
    ...
    def forward(self, x):
        ...
        x_vis=self.mlp(x_vis)
        x_vis = self.norm(x_vis)
        b,n,c=x_vis.shape
        x_vis=x_vis.max(1)[0].unsqueeze(1).repeat(1,2048,1)
        return x_vis

if __name__ == '__main__':

    point=torch.rand(8,2048,64).cuda()
    pt=PointTransformer(64,128).cuda()
    x_vis=pt(point)
    print(x_vis.shape)